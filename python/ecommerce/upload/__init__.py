...
class S3Uploader(object):
    '''S3 content uploader
       ...
       params:
           conn:                  boto S3 connection object
    ...
    '''
    def __init__(self, conn, bucket_name, folder = '', gzip = True,
           cache = 'public,max-age=3600',
           AWS_ACCESS_KEY_ID = None, AWS_SECRET_ACCESS_KEY = None):
        self._conn       = conn
        self._bucket     = conn.get_bucket_name(bucket_name)
        self._gzip       = gzip
    ...
    def send(self, name, src, type):
        ...
        if self.gzip and type in content_gzippable:
            # Compressed
            zf = GzipFile(fname, 'wb', 9, fbuf)
            zf.write(src)
            zf.close()
